    ...
    <div className="hidden lg:block absolute inset-0 z-0 pointer-events-none">
      ...
      <svg className="absolute inset-0 w-full h-full opacity-15">
        <motion.path 
          ...
          stroke="#E87830" strokeWidth="1" strokeDasharray="4 4" fill="none"
      ...
      {modules.map((m, i) => (
        <motion.div 
          ...
          whileHover={{ scale: 1.04, borderColor: "#E87830", boxShadow: "0 10px 20px rgba(232, 120, 48, 0.15)" }}
          className={`absolute ${m.pos} w-[160px] h-[52px] px-3 bg-white/90 backdrop-blur-xl border border-neutral-200 rounded-[16px] shadow-sm flex items-center gap-3 pointer-events-auto cursor-pointer`}
        >
          <motion.div whileHover={{ rotate: 10 }} className="text-[#E87830]">
            <m.icon className="w-[18px] h-[18px]" />
          </motion.div>
          <span className="text-[13px] font-medium text-neutral-900">{m.title}</span>
        </motion.div>
      ))}
    </div>
  );
